...
//   └────────── minute (0-59)
//
// Special aliases: @hourly, @daily, @weekly, @monthly, @yearly, @every_<N>s, @every_<N>m, @every_<N>h
...
function parseExpression(expr) {
  ...
  const everySecMatch = expr.match(/^@every_(\d+)s$/);
  if (everySecMatch) {
    return { intervalMs: parseInt(everySecMatch[1]) * 1000 };
  }

  // Handle @every_<n>m shorthand (interval in minutes)
  const everyMinMatch = expr.match(/^@every_(\d+)m$/);
  if (everyMinMatch) {
    return { intervalMs: parseInt(everyMinMatch[1]) * 60 * 1000 };
  }

  // Handle @every_<n>h shorthand (interval in hours)
  const everyHourMatch = expr.match(/^@every_(\d+)h$/);
  if (everyHourMatch) {
    return { intervalMs: parseInt(everyHourMatch[1]) * 60 * 60 * 1000 };
  }
