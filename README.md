...
Bot WhatsApp berbasis **[Baileys](https://github.com/WhiskeySockets/Baileys)** dengan integrasi **OpenAI-Compatible AI API** (Qwen). Mendukung sistem plugin modular tiga tipe (command, triggered, scheduled), intent detection berbasis AI session persisten, persona AI per user dan per grup, grup sebagai channel input/output plugin, AI Agent mode untuk owner, database JSON, cron scheduler, dan anti-spam typing delay.
...
│
├── core/
│   ├── bot.js                        ← Koneksi Baileys, QR auth, reconnect, init sessions, warmup grup, cron start, expose global._sock
│   ├── pluginLoader.js               ← Auto-load command plugin + daftarkan cron job dari plugin
│   ├── scheduledPluginLoader.js      ← Auto-load scheduled plugin + daftarkan cron job ke cronService
│   ├── messageHandler.js             ← Router: grup/DM | command → plugin | owner → parallel(intent+AI) | user → AI
│   └── triggeredPluginHandler.js     ← Load triggered plugin, routing berdasarkan intent JSON, intercept reply → inject ke chat session
│
...
│       └── economicNews.js           ← Kirim berita ekonomi global setiap 4 jam
│
├── services/
...
Bot start
    ...
    ├─ loadScheduledPlugins()      → load semua scheduled plugin + daftarkan cron job ke cronService
    └─ startBot()

connection.open
    ├─ global._sock = sock         → expose socket ke scheduled plugin
    ...
    └─ cronService.startAll()      → jalankan semua cron job (command plugin + scheduled plugin)
...
connection.close
    ├─ global._sock = null         → scheduled plugin tidak bisa kirim saat offline
...
Scheduled plugin berjalan **otomatis berdasarkan jadwal** tanpa trigger dari pesan apapun. Di-load oleh `scheduledPluginLoader.js` saat bot start — tidak memiliki command handler.

Scheduled plugin dapat mengakses `aiService` untuk generate konten dari Qwen, `groupService` untuk mendapat grup tertaut sebagai output, `db` untuk menyimpan data, dan `global._sock` untuk mengirim pesan ke WhatsApp.
  ...
  crons: [
    {
      ...
      expr: '@every_4h',          // @every_<N>s / @every_<N>m / @every_<N>h / @daily / cron 5-field
      handler: async () => { },
      runOnStart: false,           // true → langsung jalan saat bot start
    },
  ],
};
```

**Mengirim pesan dari scheduled plugin** menggunakan `global._sock` (di-expose oleh `bot.js` saat `connection.open`, di-clear saat `connection.close`):

```js
const targetGroups = getGroupOutput('pluginKey');
if (targetGroups.length > 0) {
  ...
  // fallback ke owner
  await global._sock?.sendMessage(config.ownerLid || config.ownerJid, { text: content });
}
```

**Scheduled plugin yang tersedia:**

| Plugin | File | Jadwal | Output |
|--------|------|--------|--------|
| Economic News | `scheduled/economicNews.js` | Setiap 4 jam | Grup tertaut atau DM owner |

**Menambah scheduled plugin baru:** lihat [PLUGIN_SETUP.md](./PLUGIN_SETUP.md)
...
Plugin dapat mendeklarasikan cron job langsung di export default via field `crons`. Job didaftarkan otomatis oleh `pluginLoader` (command plugin) atau `scheduledPluginLoader` (scheduled plugin) dan dijalankan setelah koneksi WhatsApp terbuka.
...
**Alias yang tersedia:** `@hourly`, `@daily`, `@weekly`, `@monthly`, `@yearly`, `@every_<N>s`, `@every_<N>m`, `@every_<N>h`
