...
export async function startBot() {
  // Load command plugins sebelum koneksi
  const plugins = await loadPlugins();

  // Load scheduled plugins — register cron job ke cronService
  await loadScheduledPlugins();
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      // Clear global sock — scheduled plugin tidak bisa kirim saat offline
      global._sock = null;
    ...
    if (connection === 'open') {
      ...
      // Expose sock ke global agar scheduled plugin bisa kirim pesan
      // tanpa perlu passing sock sebagai parameter ke setiap cron handler
      global._sock = sock;
