...
### Added
- `utils/whatsappFormatter.js` — utility baru untuk mengonversi output AI (markdown) ke format teks yang kompatibel dengan WhatsApp. Konversi yang didukung: heading `#` → `*teks*`, bold `**` / `__` → `*teks*`, italic `*` / `_` → `_teks_`, strikethrough `~~` → `~teks~`, bullet `- / * / +` → `•`, blockquote `>` → `❝`, horizontal rule → `▬▬▬▬▬▬▬▬▬`, tabel markdown → list bullet, link `[teks](url)` → `teks (url)`, tag HTML → dihapus, baris kosong berlebih → maksimal 2 newline. Code block dan inline code diproteksi dari transformasi agar tetap utuh.

### Changed
- `core/messageHandler.js` — fungsi `reply` diperbarui: setiap teks yang akan dikirim ke user kini diproses terlebih dahulu oleh `formatForWhatsApp()` sebelum masuk ke `typingDelay` dan `sendMessage`. Dengan satu titik integrasi ini, semua output — AI chat, command plugin, pesan error — otomatis terformat rapi tanpa perubahan di file lain.
- `config/persona.json` — ditambahkan instruksi formatting di akhir persona `owner` dan `default`: AI diarahkan untuk menggunakan format WhatsApp native (`*bold*`, `_italic_`, `•` untuk list, baris kosong antar bagian) langsung pada output-nya, sehingga teks sudah terstruktur dari sumbernya sebelum sampai ke layer formatter.

### Notes
- Pendekatan dua lapis: system prompt menginstruksikan AI output format WhatsApp secara langsung, sementara `whatsappFormatter.js` berperan sebagai safety net untuk menangani sisa markdown yang mungkin masih muncul.
